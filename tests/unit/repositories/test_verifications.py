...
class TestVerificationRepository:
    ...
    async def test_get_session_returns_data_when_exists(self, test_email: str) -> None:
        redis_mock = AsyncMock()
    ...
    async def test_create_session_uses_pipeline(
        self, redis_with_pipeline: tuple[AsyncMock, AsyncMock], test_email: str
    ) -> None:
    ...
    async def test_save_token(self, test_email: str) -> None:
        redis_mock = AsyncMock()
        repo = VerificationRepository(redis=redis_mock)
        data = VerificationTokenData(
            contact=test_email, action_type=VerificationActionType.USER_CREATION
        )
    ...
    async def test_get_token_returns_data_when_exists(self, test_email: str) -> None:
        redis_mock = AsyncMock()
