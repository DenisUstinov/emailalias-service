...
class TestVerificationServiceCreate:
    async def test_success_creates_new_session(self) -> None:
        ...
        repo_mock.get_session_id_by_email_hash.return_value = None
        repo_mock.increment_rate_limit.return_value = 1

        service = VerificationService(verification_repo=repo_mock, email_sender=email_sender_mock)

        with (
            patch("app.services.verifications.hash_email", return_value="hash"),
        ...
        email_sender_mock.send_otp.assert_awaited_once_with("test@example.com", "123456")
    ...
    async def test_success_resends_otp_updates_session(self) -> None:
        ...
        repo_mock.get_session_id_by_email_hash.return_value = "sess_id"
...
class TestVerificationServiceVerifyOperationToken:
    ...
    async def test_raises_email_not_verified_when_token_missing(self) -> None:
        repo_mock = AsyncMock()
        email_sender_mock = AsyncMock()
        ...
        with (
            patch("app.services.verifications.hash_token", return_value="hashed_token"),
            pytest.raises(EmailNotVerifiedError) as exc_info,
        ):
            await service.verify_operation_token(
                "raw_token", "test@example.com", VerificationActionType.USER_CREATION
            )
    ...
    async def test_raises_email_not_verified_when_action_type_mismatch(self) -> None:
        ...
        token_data = VerificationTokenData(
            email="test@example.com", action_type=VerificationActionType.PASSWORD_RESET
        )
        repo_mock.get_token.return_value = token_data
    ...
    async def test_raises_email_not_verified_when_email_mismatch(self) -> None:
        ...
        token_data = VerificationTokenData(
            email="other@example.com", action_type=VerificationActionType.USER_CREATION
        )
        repo_mock.get_token.return_value = token_data
